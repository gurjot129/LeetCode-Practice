...
public class Solution {
    ...
    private void justifySentence(int maxWidth, boolean isLastWord) {
        ...
        if(wordsPerSentence.size() == 1) {
            textJustification.add( sentence.append(wordsPerSentence.getFirst()).append(" ".repeat(spaceCount)).toString() );

        } else {

            //calculates the space between each word, including the additional space that will be added from left to right
            if(!isLastWord) {
                minSpaceBetweenWords = spaceCount / (wordsPerSentence.size() - 1);
                additionalSpaceBetweenWords = spaceCount % (wordsPerSentence.size() - 1);
            }
            ...
            for (int i = 0; i < wordsPerSentence.size(); ++i) {
                if (additionalSpaceBetweenWords > 0) {
                    ...
                    if(i < wordsPerSentence.size() - 1)
                        sentence.append(wordsPerSentence.get(i)).append(isLastWord ? " " : " ".repeat(minSpaceBetweenWords));
                    else
